...
class StudentAgent:
    ...
    def update_state(
        ...
        delta = (
            (self.interest * self.ability * info_level * 0.08)
            - stressor
            + coping
        )
...
tampilkan_ambang = st.sidebar.checkbox(
    "Tampilkan ambang keputusan (0.8)",
    value=True
)

tampilkan_label = st.sidebar.checkbox(
    "Tampilkan label angka pada grafik",
    value=True
)
...
run_btn = st.sidebar.button(
    "▶️ Jalankan Simulasi 1000 Iterasi"
)
...
st.caption(
    "Dashboard ini otomatis memproses hasil Monte Carlo "
    "1000 iterasi dari algoritma spasial berbasis agen."
)
            ...
            for agent in agents:
                ...
                neighbors = [
                    ...
                    if (
                        ...
                        and (
                            abs(other_agent.x - agent.x)
                            + abs(other_agent.y - agent.y)
                        ) <= 2
                    )
                ]
...
if tampilan_terpilih == "Ringkasan":
    ...
    st.subheader(
        "📋 Ringkasan Analisis Struktur Keputusan Kasus"
    )
...
# =========================================================
    ...
    st.subheader(
        "📊 Grafik Konvergensi Fluktuasi Status Siswa"
    )
    ...
    fig_bar = px.bar(
        ...
        title=(
            "Distribusi Pemilihan Jurusan Akhir"
        ),
